...
export default {
  ...
  computed: {
    ...
    whatsappTemplateCooldownActive() {
      const messages = this.currentChat?.messages || [];
      const twentyFourHoursAgo = Date.now() - 24 * 60 * 60 * 1000;
      return messages.some(msg => {
        if (msg.message_type !== 1 && msg.message_type !== 3) return false;
        if (!msg.additional_attributes?.template_params) return false;
    ...
    showWhatsappTemplates() {
      ...
      if (this.whatsappTemplateCooldownActive) return false;
