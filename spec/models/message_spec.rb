...
RSpec.describe Message do
  ...
  describe 'whatsapp template cooldown validation' do
    ...
    it 'blocks a new template message within 24 hours' do
      ...
      message = build(:message, conversation: conversation, account: account, message_type: :outgoing,
                                additional_attributes: { template_params: template_params })

      expect(message.valid?).to be(false)
      expect(message.errors[:base]).to include('WhatsApp template messages are blocked for 24 hours after the last template message.')
    end

    it 'allows a new template message after 24 hours' do
      create(:message, conversation: conversation, account: account, message_type: :outgoing, created_at: 25.hours.ago,
                       additional_attributes: { template_params: template_params })
