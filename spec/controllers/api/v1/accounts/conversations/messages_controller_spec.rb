...
RSpec.describe 'Conversation Messages API', type: :request do
  ...
  describe 'POST /api/v1/accounts/{account.id}/conversations/<id>/messages' do
    ...
    context 'when it is an authenticated user with access to conversation' do
      ...
      context 'when conversation is in WhatsApp inbox' do
        ...
        it 'rejects a second template in less than 24 hours' do
          create(:message, message_type: :outgoing, conversation: conversation, account: account,
                           additional_attributes: { template_params: template_params })

          post api_v1_account_conversation_messages_url(account_id: account.id, conversation_id: conversation.display_id),
               params: { content: 'new template', template_params: template_params },
               headers: agent.create_new_auth_token,
               as: :json

          expect(response).to have_http_status(:unprocessable_entity)
          expect(response.parsed_body['error']).to include('WhatsApp template messages are blocked for 24 hours')
        end

        it 'allows a template when the previous one is older than 24 hours' do
          create(:message, message_type: :outgoing, conversation: conversation, account: account, created_at: 25.hours.ago,
                           additional_attributes: { template_params: template_params })
