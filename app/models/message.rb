...
class Message < ApplicationRecord
  ...
  WHATSAPP_TEMPLATE_COOLDOWN_PERIOD = 24.hours
  ...
  validate :validate_whatsapp_template_cooldown, if: :outgoing_whatsapp_template_message?
  ...
  def outgoing_whatsapp_template_message?
    return false unless outgoing? || template?
    return false if additional_attributes.blank?
    return false if additional_attributes['template_params'].blank?

    channel = inbox.channel
    channel.is_a?(Channel::Whatsapp) || (channel.is_a?(Channel::TwilioSms) && channel.whatsapp?)
  end

  def validate_whatsapp_template_cooldown
    recent_template_exists = conversation.messages
                                         .where(message_type: [Message.message_types[:outgoing], Message.message_types[:template]])
                                         .where.not(id: id)
                                         .where("additional_attributes ? 'template_params'")
                                         .where('created_at >= ?', WHATSAPP_TEMPLATE_COOLDOWN_PERIOD.ago)
                                         .where.not(status: Message.statuses[:failed])
                                         .exists?

    return unless recent_template_exists

    errors.add(:base, 'WhatsApp template messages are blocked for 24 hours after the last template message.')
  end
