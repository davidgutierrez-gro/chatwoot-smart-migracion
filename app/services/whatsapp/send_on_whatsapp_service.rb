class Whatsapp::SendOnWhatsappService < Base::SendOnChannelService
  ...
  def send_template_message
    ...
    log_service = Whatsapp::TemplateLogService.new(
      message: message,
      template_name: name,
      template_params: template_params
    )
    ...
    if name.blank?
      ...
      log_service.log_failure(error_message: 'Template not found or invalid template name')
      return
    end
    ...
    if message_id.present?
      ...
      log_service.log_failure(
        error_code: message.external_error.to_s[/\d+/]&.to_i,
