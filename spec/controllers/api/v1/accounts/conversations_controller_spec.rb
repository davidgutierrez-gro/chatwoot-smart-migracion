...
RSpec.describe 'Conversations API', type: :request do
  ...
  describe 'POST /api/v1/accounts/{account.id}/conversations' do
    ...
    context 'when it is an authenticated user' do
      ...
      context 'when it is an authenticated user who has access to the inbox' do
        ...
        it 'ignores assignee and team when creator is not an administrator' do
          allow(Rails.configuration.dispatcher).to receive(:dispatch)
          post "/api/v1/accounts/#{account.id}/conversations",
               headers: agent.create_new_auth_token,
          ...
          expect(response).to have_http_status(:success)
          response_data = JSON.parse(response.body, symbolize_names: true)
          created_conversation = account.conversations.find_by(display_id: response_data[:id])

          expect(created_conversation.assignee_id).to be_nil
          expect(created_conversation.team_id).to be_nil
        end

        it 'creates a new conversation with assignee and team for administrator' do
  ...
  describe 'POST /api/v1/accounts/{account.id}/conversations/:id/toggle_status' do
    ...
    context 'when it is an authenticated user' do
      ...
      it 'does not self assign when agent changes the conversation status to open' do
        conversation.update!(status: 'pending')
        post "/api/v1/accounts/#{account.id}/conversations/#{conversation.display_id}/toggle_status",
             headers: agent.create_new_auth_token,
             as: :json
      ...
      it 'self assigns when administrator changes the conversation status to open' do
        conversation.update!(status: 'pending')
  ...
  describe 'DELETE /api/v1/accounts/{account.id}/conversations/:id' do
    ...
    context 'when it is an authenticated administrator' do
      ...
      it 'cannot delete conversations from inboxes without direct access' do
