...
describe ActionService do
  ...
  describe '#assign_agent' do
    ...
    it 'does not assign when current user is not an administrator' do
      Current.user = agent
      Current.account = account
      Current.account_user = agent.account_users.find_by(account: account)
    ...
    it 'assigns when current user is an administrator' do
      inbox_member
      Current.user = administrator
      Current.account = account
      Current.account_user = administrator.account_users.find_by(account: account)
  ...
  describe '#assign_team' do
    ...
    it 'does not change team when current user is not an administrator' do
      Current.user = agent
      Current.account = account
      Current.account_user = agent.account_users.find_by(account: account)
      original_team = conversation.team
      ...
      expect(conversation.reload.team).to eq(original_team)
    ensure
      Current.reset
    end

    it 'changes team when current user is an administrator' do
      Current.user = administrator
      Current.account = account
      Current.account_user = administrator.account_users.find_by(account: account)
      original_team = conversation.team
      ...
      expect(conversation.reload.team).not_to eq(original_team)
    ensure
      Current.reset
    end
  end
